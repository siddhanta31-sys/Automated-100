...
def run_cycle(manual=False):
    ...
    # Critical reliability fix: scheduled worker and Streamlit button can never run a cycle simultaneously.
    with cycle_lock(blocking=False) as acquired:
        if not acquired:
            active=active_cycle()
            _log(f'cycle request skipped because another cycle is active: #{active.get("id") if active else "unknown"}',None,'single_cycle_guard')
            return active.get('id') if active else None
        if not manual and not get_bool_setting('auto_enabled',True):
            _log('autonomous generation is paused by Live Studio Controls',None,'automation_paused')
            return None
        ...
        stage='startup'; _log('cycle started',cycle_id,stage)
        try:
            ...
            stage='research'; update_cycle(cycle_id,stage=stage,note='Research started'); _log(f'calling text model {TEXT_MODEL} with automatic retry/fallback',cycle_id,stage)
            ...
            stage='concept_discovery'; update_cycle(cycle_id,stage=stage,note='Generating concept pool with retry + no-hang guard')
            ...
            update_cycle(cycle_id,concepts_discovered=len(concepts),stage='scoring',note=f'Concept discovery completed: {len(concepts)} concepts')
            stage='scoring'; scored=score_concepts(concepts); update_cycle(cycle_id,candidates_scored=len(scored),note=f'Scoring completed: {len(scored)} candidates')
            ...
            stage='rendering'; update_cycle(cycle_id,stage=stage,note=f'Rendering {len(shortlist)} concepts; automatic per-image retry enabled')
            ...
            with ThreadPoolExecutor(max_workers=concurrency) as ex:
                ...
                for fut in as_completed(futs):
                    ...
                    update_cycle(cycle_id,rendered=rendered,visible=visible,rejected=rejected,failed=failed,estimated_cost_usd=EST_TEXT_CYCLE_COST_USD+rendered*EST_IMAGE_COST_USD,note=f'Progress: {rendered} rendered, {failed} failed')
            update_cycle(cycle_id,status='completed',stage='complete',finished_at=now_iso(),rendered=rendered,visible=visible,rejected=rejected,failed=failed,note='Cycle completed successfully; automatic retries/guards active.')
            return cycle_id
        except Exception as e:
